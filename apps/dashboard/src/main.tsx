...
// The native shell loads the same bundle at `index.html?surface=settings` into
...
// (NIC-148), at `index.html?surface=modemenu` into the transparent mode-swap
// dropdown that layers above open windows (NIC-144), and at `index.html?surface=sidebar`
// into the left-edge sidebar that reveals on a hover-dwell at the screen edge;
// everywhere else renders the full dashboard. The surfaces are loaded by *dynamic* import so
...
if (surface === "settings") {
  ...
  void import("./app/SidebarApp").then(({ SidebarApp }) => {
    root.render(
      <React.StrictMode>
        <SidebarApp />
      </React.StrictMode>
