...
/// PKCE (RFC 7636) primitives for the Authorization Code flow (NIC-133). A native desktop app
/// cannot safely hold a client secret, so PKCE replaces it: a high-entropy `code_verifier` is kept
/// locally, and its SHA-256 `code_challenge` travels in the authorize request. Spotify later
/// requires the original verifier at token-exchange time, proving the same client that started the
/// flow is finishing it. `state` is a separate CSRF nonce the coordinator checks on the callback.
public enum SpotifyPKCE {
    ...
    /// transform Spotify recomputes and compares at token exchange.
    public static func challenge(for verifier: String) -> String {
        let digest = CryptoKit.SHA256.hash(data: Data(verifier.utf8))
        return base64URLEncode(Data(digest))
    }

    /// A random CSRF `state` nonce (16 bytes, base64url) round-tripped through the authorize request
    /// and verified on the redirect callback.
...
public struct SpotifyTokenExchange {
    ...
    private static func tokenRequest(host: String, parameters: [String: String]) -> URLRequest? {
        ...
        request.httpBody = formURLEncoded(parameters)
        return request
    }

    /// `application/x-www-form-urlencoded` body with each key and value percent-encoded against the
    /// unreserved set and sorted for a deterministic order (aids testing; order is irrelevant to the
    /// server).
