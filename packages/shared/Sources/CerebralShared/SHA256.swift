...
///
/// CryptoKit is unavailable off Apple platforms and swift-crypto does not build
/// cleanly on the Windows toolchain this foundation targets, so confirmation plan
/// hashing (FR-SAF-05) uses this vendored implementation. It is a direct
/// transcription of FIPS 180-4 and is covered by the standard test vectors.
public enum SHA256 {
    /// Lowercase 64-character hex digest of a UTF-8 string.
    public static func hexDigest(of string: String) -> String {
        hexDigest(of: Array(string.utf8))
    }

    /// Lowercase 64-character hex digest of raw bytes.
