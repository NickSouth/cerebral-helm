...
Submitting `Ask Heimlich` opens the active Heimlich conversation in the center panel. This top-center bar is a **persistent global launcher**: it stays visible even during an active conversation and dispatches independently of the open chat — starting a new Heimlich exchange, opening an app, or running a system command. It is a **distinct surface** from the in-conversation input docked at the bottom of the center (see 5.7), which continues the *current* conversation. The two inputs coexist with distinct roles (top = global / new / unrelated; bottom = continue the current exchange). Voice activation follows the same conversation state when visible dialogue is required.

**Floating command palette (owner decision, supersedes the earlier "no floating modal" stance; NIC-75 / FR-SHL-02).** In the native macOS shell a configurable global hotkey (default ⌥Space) and a menu-bar item summon a floating, Spotlight-style command palette as its **own native window** (an `NSPanel`) that appears over whatever app is frontmost. The palette hosts the *same* `CommandSurface` (a lightweight `index.html?surface=palette` WebKit route) and submits through the *same* versioned bridge as the dashboard — it is not a parallel input. Executing a command dismisses the palette; Escape and click-away dismiss it; repeated invocation focuses the single existing palette rather than opening a second. "Ask Heimlich" from the palette routes the conversation to the dashboard's center panel (window-role choreography, NIC-76). This palette window is the one sanctioned floating surface; the launcher/docked inputs above remain the in-dashboard surfaces.
