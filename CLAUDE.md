...
- **Remaining NIC-85 increments:** 12–13 `apps.list` discovery + pinning through the config-write path (NIC-119; wires the override read side into bootstrap) → 14 login item + single instance (NIC-89). Increments 9–11 (display detection, backdrop, multi-display + main-display setting) are built and uncommitted, with the strict-backdrop re-scope (dashboard never lifts; native confirmation panel + settings window — reverses NIC-76 overlay-only). Increment designs and machine quirks live in the auto-memory (`nic85-increment-progress`).
