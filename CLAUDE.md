...
- **Remaining NIC-85 increments:** 13 pinning through the config-write path (NIC-119c; wires the override read side into bootstrap) → 14 login item + single instance (NIC-89). Increments 9–12 (display detection, backdrop, multi-display + main-display setting + companion surface, `apps.list` discovery + More Apps picker) are built and uncommitted, with the strict-backdrop re-scope (dashboard never lifts; native confirmation panel + settings window — reverses NIC-76 overlay-only). Increment designs and machine quirks live in the auto-memory (`nic85-increment-progress`).
