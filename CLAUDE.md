...
- **Remaining NIC-85 increments:** 11 multi-display backdrops + main-display setting (NIC-120b) → 12–13 `apps.list` discovery + pinning through the config-write path (NIC-119; wires the override read side into bootstrap) → 14 login item + single instance (NIC-89). Increment designs and machine quirks live in the auto-memory (`nic85-increment-progress`).
